"""Add SaltStack tables for minions, events, service states, processes, packages, and logs.
...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column('source', sa.String(), nullable=False),  # apt, yum, dnf, pacman, etc.
...
def downgrade():
    ...
    op.drop_index('idx_service_states_status', 'salt_service_states')
    op.drop_index('idx_service_states_service_name', 'salt_service_states')
    op.drop_index('idx_service_states_server_id', 'salt_service_states')
